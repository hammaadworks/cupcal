...
export default function ScheduleFilter({ matches, initialMatchId }: FilterProps) {
  ...
  const effectiveSelectedTeams = useMemo(() => {
    ...
    if (globalTeam) teams.add(globalTeam);
  ...
  const groupedMatches = useMemo(() => {
    ...
    if (effectiveSelectedTeams.length > 0) {
      filtered = filtered.filter(m => (m.home && effectiveSelectedTeams.includes(m.home)) || (m.away && effectiveSelectedTeams.includes(m.away)));
    }
  ...
  }, [matches, $timezone, isMounted, effectiveSelectedTeams, selectedVenues, selectedStages, selectedGroups, selectedDate]);
  ...
  const hasAdvancedFilters = effectiveSelectedTeams.length > 0 || selectedVenues.length > 0 || selectedStages.length > 0 || selectedGroups.length > 0;
  ...
  if (groupedMatches.length === 0 && !hasAdvancedFilters) {
     return <div className="text-center text-black font-outfit text-2xl py-12 uppercase tracking-wider">VIBE CHECKING SCHEDULE...</div>;
  }
  ...
  return (
    <div className="w-full max-w-4xl mx-auto space-y-12 font-sans mt-12 pb-32 px-2 md:px-0">
        ...
        */}
        ...
        <div className="bg-white border-[4px] border-black p-4 md:p-6 shadow-[8px_8px_0px_#2E0D23] relative z-20 flex flex-col gap-4">
           ...
           <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b-[4px] border-black pb-4">
              <div className="flex flex-col">
                 <h2 className="text-3xl md:text-5xl font-outfit text-black uppercase tracking-tight leading-none">FIND YOUR VIBE</h2>
          ...
          {hasAdvancedFilters && (
             <div className="flex flex-wrap gap-3 items-center">
                <span className="font-outfit text-sm uppercase tracking-widest text-black">ACTIVE:</span>
                {effectiveSelectedTeams.map(t => (
                   <span key={t} className="bg-black text-white px-3 py-1 rounded-full text-xs font-outfit uppercase tracking-widest border-[2px] border-black flex items-center gap-2 shadow-[2px_2px_0px_#2E0D23]">
                      {getTeamName(t)} 
                      {globalTeam !== t && <button onClick={() => toggleSelection(setSelectedTeams, t)} className="hover:text-blue-600">✕</button>}
                   </span>
                ))}
                {selectedVenues.map(v => (
                   <span key={v} className="bg-black text-white px-3 py-1 rounded-full text-xs font-outfit uppercase tracking-widest border-[2px] border-black flex items-center gap-2 shadow-[2px_2px_0px_#2E0D23]">
                      {v} 
                      <button onClick={() => toggleSelection(setSelectedVenues, v)} className="hover:text-blue-600">✕</button>
                   </span>
                ))}
                {selectedStages.map(s => (
                   <span key={s} className="bg-black text-white px-3 py-1 rounded-full text-xs font-outfit uppercase tracking-widest border-[2px] border-black flex items-center gap-2 shadow-[2px_2px_0px_#2E0D23]">
                      {s} 
                      <button onClick={() => toggleSelection(setSelectedStages, s)} className="hover:text-blue-600">✕</button>
                   </span>
                ))}
                {selectedGroups.map(g => (
                   <span key={g} className="bg-black text-white px-3 py-1 rounded-full text-xs font-outfit uppercase tracking-widest border-[2px] border-black flex items-center gap-2 shadow-[2px_2px_0px_#2E0D23]">
                      {g} 
                      <button onClick={() => toggleSelection(setSelectedGroups, g)} className="hover:text-blue-600">✕</button>
                   </span>
                ))}
                <button onClick={() => { setSelectedTeams([]); setSelectedVenues([]); setSelectedStages([]); setSelectedGroups([]); }} className="text-xs font-outfit uppercase text-blue-700 hover:text-black underline tracking-widest ml-2">
                   CLEAR ALL
                </button>
             </div>
          )}
        ...
        <div id="sticky-header" className="sticky top-16 z-40 bg-[#F7803C]/95 backdrop-blur-xl pb-2 pt-2 -mx-2 px-2 md:mx-0 md:px-0 mb-8 border-transparent shadow-[0_4px_10px_rgba(0,0,0,0.1)] border-none transition-all duration-300 rounded-b-2xl md:rounded-2xl">

        <details 
          className="group bg-white border-[4px] border-black shadow-[4px_4px_0px_#2E0D23] mb-4 rounded-xl overflow-hidden transition-all"
          open={filtersOpen}
          onClick={(e) => {
            e.preventDefault();
        ...
        >
          <summary className="p-3 md:p-4 font-outfit tracking-widest uppercase text-base md:text-lg cursor-pointer bg-yellow-300 hover:bg-blue-300 list-none flex justify-between items-center transition-colors border-b-[4px] border-transparent group-open:border-black">
            <div className="flex items-center gap-2">
              <span className="text-xl">🔍</span>
              <span>{hasAdvancedFilters ? 'FILTERS APPLIED (TAP TO EDIT)' : 'SEARCH & FILTERS'}</span>
            </div>
            <span className={`transition-transform bg-white rounded-full w-6 h-6 flex items-center justify-center border-[2px] border-black shadow-[2px_2px_0px_#2E0D23] text-sm ${filtersOpen ? 'rotate-180' : ''}`}>▼</span>
          </summary>
          ...
          <div className="p-4 space-y-6 bg-gray-50 max-h-[60vh] overflow-y-auto">
             ...
             <div className="space-y-3">
                <div className="flex justify-between items-center px-2 gap-2 md:gap-4">
                   <h3 className="text-sm md:text-xl font-outfit text-black tracking-widest uppercase bg-yellow-300 px-2 md:px-4 py-1 border-[2px] border-black shadow-[2px_2px_0px_#2E0D23] inline-block shrink-0">SQUADS</h3>
                   <div className="relative flex-1 max-w-[300px]">
                     <input 
                       ...
                       className="w-full text-sm font-outfit tracking-widest px-4 py-2 bg-white border-[3px] border-black rounded-full shadow-[2px_2px_0px_#2E0D23] focus:outline-none focus:bg-blue-100 placeholder-gray-400 transition-all uppercase"
                     />
                     {squadSearch && (
                       <button onClick={() => setSquadSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 bg-black text-white rounded-full w-5 h-5 flex items-center justify-center font-bold text-xs hover:bg-blue-600 hover:scale-110 transition-all">✕</button>
                     )}
                   </div>
                </div>
                <div className="flex overflow-x-auto custom-scrollbar gap-4 pb-4 px-2 snap-x" style={{ maskImage: 'linear-gradient(to right, transparent, black 15px, black calc(100% - 15px), transparent)', WebkitMaskImage: 'linear-gradient(to right, transparent, black 15px, black calc(100% - 15px), transparent)' }}>
                   {(() => {
                      ...
                      return filteredTeams.map(teamCode => {
                         const isActive = effectiveSelectedTeams.includes(teamCode);
                         ...
                         return (
                            <button 
                               ...
                               className={`snap-center flex-shrink-0 flex flex-col items-center gap-2 p-3 w-24 rounded-xl border-[3px] border-black transition-all ${isActive ? 'bg-black shadow-[4px_4px_0px_#F7803C] scale-105' : 'bg-white hover:bg-gray-100 shadow-[2px_2px_0px_#2E0D23]'}`}
                            >
                               ...
                               <span className={`text-[11px] font-outfit uppercase text-center w-full truncate ${isActive ? 'text-blue-500' : 'text-black'}`}>{name}</span>
                            </button>
                         );
                      });
                   })()}
             ...
             <div className="pt-4 border-t-[4px] border-black border-dashed">
               <h4 className="font-outfit tracking-widest uppercase text-base md:text-lg mb-4 text-blue-700">ADVANCED: STAGES, GROUPS, TURFS</h4>
        ...
        {!hasAdvancedFilters && (
          <div className="relative">
             <div className="flex justify-between px-2 mb-2 items-center">
                 <span className="font-outfit text-sm uppercase tracking-widest text-black">MATCH DAYS</span>
                 <div className="flex gap-2 items-center">
                   ...
                   }} className="text-[10px] sm:text-xs font-outfit uppercase bg-[#FFEDBF] text-black px-3 py-1 rounded-full border-[2px] border-black hover:bg-yellow-400 hover:scale-105 transition-transform shadow-[2px_2px_0px_#2E0D23]">
                 ...
                 }} className="text-[10px] sm:text-xs font-outfit uppercase bg-black text-white px-3 py-1 rounded-full border-[2px] border-black hover:bg-blue-600 hover:scale-105 transition-transform shadow-[2px_2px_0px_#2E0D23]">
                    JUMP TO TODAY
                 </button>
                 </div>
             </div>
              ...
              return (
                <button
                  ...
                  className={`snap-center relative flex-shrink-0 flex flex-col items-center justify-center w-[72px] h-[88px] rounded-[1rem] border-[3px] border-black transition-all duration-200 ${
                    isActive ? 'bg-black text-white shadow-[4px_4px_0px_#2E0D23] scale-105' : 'bg-white text-black hover:bg-gray-100 shadow-[2px_2px_0px_#2E0D23]'
                  }`}
                >
                  {isToday && <div className="absolute -top-2 -right-2 w-4 h-4 bg-red-500 rounded-full border-[2px] border-black animate-pulse shadow-[2px_2px_0px_#2E0D23]" />}
                  <span className={`text-[11px] font-outfit uppercase tracking-wider mb-1 ${isActive ? 'text-white' : 'text-black'}`}>{dayStr}</span>
        ...
        <div className="space-y-12">
            ...
            return (
              <div key={group.date} id={`group-${group.date}`} data-date={group.date} className="date-group space-y-4 pt-4">
                <div className="relative flex py-4 items-center mb-8">
                    ...
                    <span className="flex-shrink-0 mx-4 text-black font-outfit tracking-widest text-lg md:text-xl uppercase bg-white px-4 py-1 border-[2px] border-black rounded-full shadow-[2px_2px_0px_#2E0D23]">
                        {fullDate}
                    </span>
                    <div className="flex-grow border-t-[4px] border-black border-dashed opacity-50"></div>
                </div>
          ...
          <div className="md:col-span-2 space-y-4 mb-4">
            <div className="flex justify-between items-center px-1">
              <h2 className="text-3xl font-outfit text-black tracking-tight uppercase">LIVE NOW / NEXT</h2>
              <button onClick={(e) => handleDownloadSingle(e, upcomingMatch)} className="text-xs font-outfit text-white bg-black hover:bg-blue-600 hover:-translate-y-1 hover:-translate-x-1 uppercase tracking-widest px-4 py-2 rounded-full border-[3px] border-black shadow-[4px_4px_0px_#2E0D23] transition-all flex items-center gap-2">
            ...
            <div onClick={(e) => handleMatchClick(e, upcomingMatch)} role="button" tabIndex={0} className="block w-full text-left match-card relative border-[3px] border-black shadow-[8px_8px_0px_#2E0D23] rounded-tl-[3rem] rounded-br-[3rem] rounded-tr-xl rounded-bl-xl hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[10px_10px_0px_#2E0D23] transition-all overflow-hidden cursor-pointer" style={{ background: `linear-gradient(90deg, ${homeColor} 50%, ${awayColor} 50%)` }}>
              <div className="absolute top-0 left-1/2 -translate-x-1/2 mt-0 bg-black px-6 py-1 rounded-b-xl border-b-[3px] border-l-[3px] border-r-[3px] border-black z-10 flex gap-2">
                {(() => {
                  ...
                  return <div className={`${status==='LIVE'?'text-red-500 animate-pulse':status==='DONE'?'text-gray-400':'text-blue-400'} font-outfit tracking-widest text-sm uppercase`}>{status}</div>
                })()}
                <div className="text-white font-outfit tracking-widest text-sm uppercase">M{upcomingMatch.matchNumber}</div>
              </div>
              ...
              <div className="flex items-center justify-between mt-8 p-6 md:p-8 relative z-0">
                <div className="flex flex-col items-center gap-4 w-1/3">
                  {upcomingMatch.home && getTeamLogo(upcomingMatch.home) ? (
                    <img src={getTeamLogo(upcomingMatch.home)} alt={homeName} className="w-16 h-16 md:w-24 md:h-24 object-contain drop-shadow-[2px_2px_0px_#2E0D23]" onError={(e) => e.currentTarget.style.display='none'} />
                  ) : (
                    <span className="text-4xl drop-shadow-xl mb-2">🏳️</span>
                  )}
                  <span className="font-outfit text-sm sm:text-xl md:text-3xl text-center text-white drop-shadow-[2px_2px_0px_#2E0D23] uppercase break-words">{homeName}</span>
                </div>
                
                <div className="w-1/3 flex flex-col items-center justify-center">
                  <MatchCountdown dateStr={upcomingMatch.kickoffUtc} />
                  <div className="mt-6 text-[10px] md:text-sm font-outfit text-black uppercase tracking-widest bg-white px-2 py-1 md:px-4 rounded-full border-[2px] border-black text-center shadow-[2px_2px_0px_#2E0D23]">
                ...
                <div className="flex flex-col items-center gap-4 w-1/3">
                  {upcomingMatch.away && getTeamLogo(upcomingMatch.away) ? (
                    <img src={getTeamLogo(upcomingMatch.away)} alt={awayName} className="w-16 h-16 md:w-24 md:h-24 object-contain drop-shadow-[2px_2px_0px_#2E0D23]" onError={(e) => e.currentTarget.style.display='none'} />
                  ) : (
                    <span className="text-4xl drop-shadow-xl mb-2">🏳️</span>
                  )}
                  <span className="font-outfit text-sm sm:text-xl md:text-3xl text-center text-white drop-shadow-[2px_2px_0px_#2E0D23] uppercase break-words">{awayName}</span>
                </div>
              </div>
            </div>
          </div>
