...
export default function StatusGenerator({ match }: { match: Match }) {
  ...
  const generateCanvas = () => {
    ...
    if (template === 'B') {
      ...
      gradient.addColorStop(1, '#FFEDBF');
    }
    ...
    if (template === 'B') {
      // MATCH DAY TEMPLATE
      ctx.fillStyle = '#2E0D23000';
      ctx.font = '160px Outfit, sans-serif';
      ctx.fillText(match.homeTeam.toUpperCase(), 540, 500);
      
      ctx.font = '120px Outfit, sans-serif';
      ctx.fillStyle = '#3b82f6'; // Blue VS
      ...
      ctx.fillStyle = '#2E0D23000'; 
      ctx.font = '140px Outfit, sans-serif';
      ...
      ctx.fillStyle = '#2E0D23000';
      ctx.font = '70px Outfit, sans-serif';
  ...
  return (
    <div className="bg-white p-6 md:p-8 rounded-[2rem] border-[4px] border-black mt-8 shadow-[8px_8px_0px_#2E0D23]">
      <div className="text-center mb-6">
        <h2 className="text-2xl md:text-3xl font-outfit text-black tracking-widest uppercase mb-2">Match Story Asset</h2>
        <p className="text-black font-outfit uppercase text-sm md:text-base tracking-widest">Download a high-quality graphic for social media.</p>
      </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center">
        <div className="w-full md:w-1/2 flex flex-col gap-3">
          <button 
            ...
            className={`p-4 rounded-xl text-left transition-all border-[3px] shadow-[4px_4px_0px_#2E0D23] hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[6px_6px_0px_#2E0D23] ${template === 'B' ? 'border-black bg-blue-300' : 'border-black bg-white'}`}
          >
            <h3 className="font-outfit text-xl text-black uppercase tracking-wider">Template: Match Day</h3>
            <p className="text-sm text-black font-medium mt-1">Rich contrast card with kick-off & stadium.</p>
          </button>
          
          <button 
            onClick={() => setTemplate('A')}
            className={`p-4 rounded-xl text-left transition-all border-[3px] shadow-[4px_4px_0px_#2E0D23] hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[6px_6px_0px_#2E0D23] ${template === 'A' ? 'border-black bg-blue-300' : 'border-black bg-white'}`}
          >
            <h3 className="font-outfit text-xl text-black uppercase tracking-wider">Template: Predictions</h3>
            <p className="text-sm text-black font-medium mt-1">Interactive "Who will win?" engagement overlay.</p>
          </button>

          <button 
            onClick={downloadImage}
            className="mt-4 w-full py-4 bg-yellow-300 text-black font-outfit text-xl rounded-xl hover:bg-blue-600 transition-all border-[4px] border-black shadow-[6px_6px_0px_#2E0D23] flex items-center justify-center gap-2 uppercase tracking-widest hover:-translate-y-1 hover:-translate-x-1 hover:shadow-[8px_8px_0px_#2E0D23]"
          >
        ...
        <div className="w-full md:w-1/2 flex justify-center bg-black p-4 rounded-2xl overflow-hidden border-[4px] border-black shadow-[6px_6px_0px_#2E0D23]">
          <canvas 
            ref={canvasRef} 
